...
namespace ConversationTracker.Controllers
{
    public class TrackerController : Controller
    {
        ...
        public JsonResult retrieveTrackedConversations()
        {
            ...
            var conversation = new ConversationTrackerObject { 
                Date = DateTime.Now,
                NotesOfChangeOverTime = "none",
                RateOfUnease = 0,
                SettingOrEnvironment = "home",
                Who = "me?!"
            };

            collection.InsertOneAsync(conversation).Wait();

            Task<List<ConversationTrackerObject>> waiting = collection.Find(x => x.Who == "me?!").ToListAsync();
            waiting.Wait();
